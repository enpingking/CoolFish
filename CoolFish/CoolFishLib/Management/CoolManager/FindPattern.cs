...
namespace CoolFishNS.Management.CoolManager
{
    ...
    public class FindPattern
    {
...
#endif
        {
            ...
            var data = ReadBytes(process, (IntPtr) startAddress, (int) (endAddress - startAddress));
...
#endif
        {
            ...
            // Get a temporary set of data to work with. :)
            var data = ReadBytes(process, baseAddress, basObject.ModuleMemorySize);
        ...
        {
            using (var reader = new InProcessMemoryReader(process))
            {
...
#endif
        {
            ...
            var pats = from p in file.Descendants("Pattern")
                select p;
            ...
            foreach (var pat in pats)
            {
                ...
                var name = pat.Attribute("desc").Value;
                var mask = pat.Attribute("mask").Value;
                var patternBytes = GetBytesFromPattern(pat.Attribute("pattern").Value);
                ...
                foreach (var e in pat.Elements())
                {
                    switch (e.Name.LocalName)
                    {
                        ...
                        case "Rel":
                            var instructionSize = uint.Parse(e.Attribute("size").Value, NumberStyles.HexNumber);
                            var operandOffset = uint.Parse(e.Attribute("offset").Value, NumberStyles.HexNumber);
#if !X64
        ...
        private static byte[] GetBytesFromPattern(string pattern)
        {
            ...
            var split = pattern.Split(new[] {'\\', 'x'}, StringSplitOptions.RemoveEmptyEntries);
